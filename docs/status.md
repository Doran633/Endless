...
当前阶段：独立网页版 AI 助手 v1.0 的最小文档解析闭环阶段。
...
- 进行 v0.4 浏览器联调：上传 TXT / DOCX / PDF 后触发解析并查看预览。
...
**前端基础原型 + 后端真实 LLM 聊天闭环 + 后端本地文件上传闭环 + 最小文档解析闭环 + v1.0 文档规划。**
...
- 文档 chunk、embedding 和 RAG 主链路尚未开始。
- 数据库和持久化能力尚未建立。
- AI 数据分析仍只是规划能力，尚未进入实现。

因此，下一阶段应先完成 v0.4 浏览器联调和 PDF 依赖验证，再进入 v0.5 文本切块；AI 数据分析继续保留规划边界，不挤占当前主链路。
