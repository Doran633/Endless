...
当前阶段：Persistent Conversations 自动会话标题阶段。
...
- 对 v1.2.4 / v1.2.5 前端恢复与自动标题进行联调和收尾提交。
- 后续再设计是否将最近若干轮历史消息带入 LLM 上下文。
...
- 聊天会话 API、普通聊天/RAG 消息落库、前端刷新恢复、当前文件绑定前端接入和自动会话标题已完成。
- AI 数据分析仍只是规划能力，尚未进入实现。

因此，下一阶段应先对 v1.2 持久会话进行联调和收尾检查，再决定是否进入基础上下文窗口或 AI 数据分析规划实现。AI 数据分析继续保留规划边界，不挤占当前主链路。
