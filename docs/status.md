...
当前阶段：独立网页版 AI 助手 v1.0 的最小文件上传闭环阶段。
...
- 文档解析、Embedding、RAG 问答、数据库持久化尚未开始实现。
...
- 进行 v0.3 手动联调：浏览器选择文件并确认 `backend/uploads/` 写入。
- 补充后端文件上传接口最小自动化测试。
- 进入 v0.4 文档解析设计与实现。
...
**前端基础原型 + 后端真实 LLM 聊天闭环 + 后端本地文件上传闭环 + v1.0 文档规划。**
...
因此，下一阶段应先完成 v0.3 浏览器联调和最小测试固化，再进入 v0.4 文档解析；AI 数据分析继续保留规划边界，不挤占当前主链路。
