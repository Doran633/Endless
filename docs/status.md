...
- 实现文件删除 API。
- 优化文件中心上传体验，让文件中心上传后可自动进入解析、切块、向量化和索引流程。
- 继续推进聊天会话和消息持久化。
...
- 数据库基础、文件元数据持久化和文件处理状态持久化已经建立，但文件删除、会话和消息持久化仍未完成。
- AI 数据分析仍只是规划能力，尚未进入实现。

因此，下一阶段应继续推进 v1.1 数据库持久化：优先补齐文件删除 API 和文件中心自动处理体验，再推进聊天会话和消息持久化。AI 数据分析继续保留规划边界，不挤占当前主链路。
